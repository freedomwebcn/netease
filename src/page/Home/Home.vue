...
export default {
  ...
  async mounted() {
    this.init()
    // 轮播图数据
    const { banner } = await reqBanner({ type: 2 })
    this.resBannersData = banner.banners
    this.$nextTick(() => {
      this._initBannerSwiper()
    })
    // 推荐歌单数据
    const { recommend_playlist } = await reqRecommendPlaylist({ limit: 6 })
    this.recommendPlaylistData = recommend_playlist.result
    // 推荐的音乐
    const { new_song } = await reqRandomSong()
    this.recommendMusicData = new_song.result
    // 热门歌单
    const { hot_playlist } = await reqHotPlaylist()
  ...
  methods: {
    async newMusicTabChange(event) {
      ...
      if (idx == 1 && !this.newCdData.length) {
        // 新碟
        const { new_cd } = await reqNewCd()
        this.newCdData = new_cd.albums
      }
      if (idx == 2 && !this.newAlbumData.length) {
        // 新专辑
        const { new_album } = await reqNewAlbum()
        this.newAlbumData = new_album.products
      }
    },
    init() {
